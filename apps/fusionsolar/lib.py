"""
fusionsolar/lib.py — Pure Python, no pyscript globals.
Called via task.executor from __init__.py.

Do NOT use: log.*, state.*, service.*, task.*, @time_trigger, etc.
"""
...
def _normalize(balance, kpi, station_dn, logs):
    ...
    if balance is None:
        ...
        if pv_kw is not None and load_kw is not None:
            if self_use is not None:
                ...
                grid_import_kw = max(
                    load_kw - self_use - (battery_discharge_kw or 0.0) + (battery_charge_kw or 0.0),
                    0.0
                )
            else:
    ...
    if pv_kw is None:       logs.append("WARNING productPower series vide ou absente")
    if battery_soc is None: logs.append("WARNING batterySoc absent du KPI")
    if grid_import_kw is None: logs.append("WARNING grid dérivé impossible (pv ou load manquant)")
...
def fetch(config):
    """
    Entry point — called via task.executor from pyscript __init__.py.
    Returns a plain dict.
    """
    logs = []
    cookie = config.get("cookie", "")
    if not cookie or cookie == "PASTE_EDGE_NETWORK_COOKIE_HERE":
        raise RuntimeError("FusionSolar cookie manquant — éditer /config/pyscript/apps/fusionsolar/__init__.py")
